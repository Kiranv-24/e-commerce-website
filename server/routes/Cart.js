...
router.post("/add", async (req, res) => {
   ...
   try {
      if (!username) return res.status(401).json({ message: "Login to your account" });

      const product = await Product.findById(productId);
      if (!product) return res.status(404).json({ message: "Product not found!" });

      const existingCart = await Cart.findOne({ username });
      const itemIndex = existingCart?.cartitems.findIndex(item => item.name === product.name);

      if (existingCart && itemIndex >= 0) {
         // If item exists, update quantity
         existingCart.cartitems[itemIndex].quantity += quantity || 1;
         await existingCart.save();
         res.status(200).json({ message: "Product quantity updated in cart", cart: existingCart });
      } else {
